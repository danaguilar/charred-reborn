  ...
  <b-row @click="toggleCollapse">
    <b-col :class="toggleBottomBorder()">
      <h4 class="d-flex justify-content-between align-items-end">
        ...
        <span><em>Cirlces</em> {{ character.GetCirclesShade() }}{{ character.GetCirclesValue() }}</span>
        <div></div>
      </h4>
    </b-col>
  </b-row>
      ...
      <table class="table small table-borderless">
        <tbody>
          <tr class="row-with-border">
            <td colspan="3" class="text-center"><em>Start with Will/2</em></td>
          </tr>
          <tr>
            <th>Will</th>
          ...
          <tr class="row-with-border">
            <td colspan="3" class="text-center"><em>Add one if sum > 50</em></td>
          </tr>
          <tr>
            <th>Property</th>
            <td class="text-end">{{ character.SumOfProperty() }}</td>
          </tr>
          <tr>
            <th>Relationships</th>
            <td class="text-end">{{ character.SumOfRelationships() }}</td>
          </tr>
        </tbody>
      </table>
    </b-col>
