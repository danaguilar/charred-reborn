  ...
  <b-row @click="toggleCollapse">
    <b-col :class="toggleBottomBorder()">
      <h4 class="d-flex justify-content-between align-items-end">
        ...
        <span><em> Mortal Wounds</em> {{ character.GetMortalWoundsShade() }}{{ character.GetMortalWoundsValue() }}</span>
        <div></div>
      </h4>
    </b-col>
  </b-row>
    ...
    <b-col>
      <table class="table small table-borderless">
        <tbody>
          ...
          <tr>
            <th>Power</th>
            <td class="text-end">{{ character.PhysicalAttributes.PowerAttr }}</td>
          </tr>
          <tr>
            <th>Forte</th>
