  ...
  <b-row @click="toggleCollapse">
    <b-col :class="toggleBottomBorder()">
      <h4 class="d-flex justify-content-between align-items-end">
        ...
        <span><em>Steel</em> {{ character.GetSteelShade() }}{{ character.GetSteelValue() }}</span>
        <div></div>
      </h4>
    </b-col>
  </b-row>
      ...
      <table class="table small table-borderless">
        <tbody>
          <tr class="row-with-border">
            <td colspan="3" class="text-center"><em>Start with</em></td>
          </tr>
          <tr>
            <th>Starting Steel</th>
            <td class="text-end">B3</td>
          </tr>
          <tr class="row-with-border">
            <td colspan="3" class="text-center"><em>Additional Questions</em></td>
          </tr>
          <tr v-for="question in character.steel_questionnaire.questions">
            <td>{{ question.text }}</td>
            <td>
